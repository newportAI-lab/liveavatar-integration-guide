...
After publishing you will receive an `avatarId` (the unique identifier for the avatar).
...
> `sessionToken` is valid for approximately 1 hour. Refresh it before each session.
...
The platform proxies calls to a developer-specified external LLM (OpenAI, Claude, Tongyi, etc.). Developers only need to enter their LLM API Key in the console — **no inference service to deploy**. The platform handles ASR, RAG retrieval, and TTS synthesis.
...
```plain
https://github.com/newportAI-lab/liveavatar-channel
```
...
```plain
https://github.com/newportAI-lab/liveavatar-channel-python
```
