...
| **Agent token** | The credential for the developer's agent to join the RTC room (WebRTC modes only). |
...
| **Agent WS URL** | In WS Inbound mode, the dynamically allocated WebSocket endpoint for this session. It embeds a one-time token bound to the current `sessionId`. For **backend use only** — do not forward to the frontend. |
